...
Token Lexer::getNextToken()
{
  ...
  if (std::isalpha(c) || c == '_')
  {
    ...
    TokenType type = checkKeyword(lexeme); // it IS a reserved word or a variable name
  ...
  if (std::isdigit(c))
  {
    ...
    // The distinction between int and float is going to be made by the semantic analysis
  ...
  case '*':
    return Token{TokenType::Asterisk, source.substr(startPos, 1), line, startCol};
  case '{':
    return Token{TokenType::OpenBrace, source.substr(startPos, 1), line, startCol};
  case '}':
    return Token{TokenType::CloseBrace, source.substr(startPos, 1), line, startCol};
  case '(':
    return Token{TokenType::OpenParen, source.substr(startPos, 1), line, startCol};
  case ')':
    return Token{TokenType::CloseParen, source.substr(startPos, 1), line, startCol};
  case ';':
    return Token{TokenType::Semicolon, source.substr(startPos, 1), line, startCol};
  case ',':
    return Token{TokenType::Comma, source.substr(startPos, 1), line, startCol};
  case '=':
    if (peek() == '=')
    {
      advance();                                                                   // Consumes second '='
      return Token{TokenType::Equals, source.substr(startPos, 2), line, startCol}; // Isso deveria ser um TokenType::Equals
    }
    return Token{TokenType::Assign, source.substr(startPos, 1), line, startCol};
  }

  // Let the parser handles erros or unknown simbols - this groups all erros together and keep the parser responsible for errors (SOLID)
...
TokenType Lexer::checkKeyword(std::string_view lexeme) const
{
  ...
  if (it != keywords.end())
    return it->second; // Reserved key word
...
constexpr TokenType checkKeywordConstexpr(std::string_view lexeme)
{
  ...
  while (left <= right)
  {
    ...
    // constexpr compare
    // Returns 0 if equals, < 0 if lexeme comes before, > 0 if lexeme comes after
