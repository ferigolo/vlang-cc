...
{
public:
  virtual ~ExprAST() = default; // Every node from the root will be correctly deleted
};
...
public:
  BinaryExprAST(char operation, std::unique_ptr<ExprAST> left, std::unique_ptr<ExprAST> right) : op(operation), lhs(std::move(left)), rhs(std::move(right)) {}
